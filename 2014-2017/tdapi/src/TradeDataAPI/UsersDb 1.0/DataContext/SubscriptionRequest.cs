        ...
        public void BeforeSave(UsersDataContext context, ObjectStateEntry entry)
        {
            ...
            if (request != null)
            {
                ...
                if (request.IsFulfilled && request.Notify == true)
                {
                    ...
                    if (request.Accepted)
                    {
                        message = eh.ComposeMessage(ServiceConfig.Smtp_Sender, request.User.Email,
                            ServiceConfig.Subscription_Request_Accepted_Notification_Subject,
                            ServiceConfig.Subscription_Request_Accepted_Notification_Body);
                    }
                    else
                    {
                        message = eh.ComposeMessage(ServiceConfig.Smtp_Sender, request.User.Email,
                            ServiceConfig.Subscription_Request_Declined_Notification_Subject,
                            ServiceConfig.Subscription_Request_Declined_Notification_Subject);
                    }
