...
namespace UsersDb.Search
{
    ...
    public class XymDataManager
    {
        ...
        /// <returns></returns>
        public static DataSet Search(SearchParametersJson prms, out int rootCount, out int seriesCount)
        {
            var ds = new DataSet();
            seriesCount = 0;
            ...
            using (var dataContext = new UsersDataContext())
            {
                ...
                var orderBy = ServiceConfig.Search_Results_Order_By_Column;
                var ascending = ServiceConfig.Search_Results_Order_By_Direction.Equals("asc", StringComparison.InvariantCultureIgnoreCase); 
                ...
                var results = se
                    ...
                    .Find(out rootCount)
                    .GetPage(prms.PageNumber, prms.PageSize, orderBy, ascending);
                ...
                if (prms.IncludeRoot)
                {
                    results.ResultsDataTable.TableName = DbHelper.GetTableName(tableNames.ToList(), "Table");
                    ds.Tables.Add(results.ResultsDataTable);
                }

                if (prms.IncludeSeries)
                {
                    var seriesAll = FindSeries(results.RawSearchResults, prms, out seriesCount);
        ...
        public static DataSet SeriesFullTextSearch(RequestParameters prms, out int count)
        {
            ...
            using (var dataContext = new UsersDataContext())
            {
                ...
                var ds = h.CallStoredProcedure(prms.StoredProcName, WorkingDatabase.Instance.GetStoredProcParamPrefix(), new Dictionary<string, object>
                {
                    {"pn", prms.SearchParameters.PageNumber},
                    {"ps", prms.SearchParameters.PageSize},
                    {"fa", prms.SearchParameters.FindAll},
                    {"words", prms.SearchParameters.Keyword.Split(',').ToTable("string")}
                },
                out count,
                28800);

                var cols = prms.SearchParameters.VisibleSeriesColumns.Count == 0
                    ? new List<string>()
                    : prms.SearchParameters.VisibleSeriesColumns.Union(KeyColumns).ToList();

                if (ds.Tables.Count > 0 && cols.Any())
                {
                    cols.ForEach(c => ds.Tables[0].Columns.Remove(c));
                }

                var series = ds.Tables.Count == 0 ? new DataTable() : ds.Tables[0];
                return ApplyRulesAndFiltersForSeries(dataContext, series, prms.SearchParameters);
            }
        }
        ...
        {
            var orderBy = ServiceConfig.Series_Order_By;
            var ascending = ServiceConfig.Series_Order_By_Direction.Equals("asc", StringComparison.InvariantCultureIgnoreCase);
