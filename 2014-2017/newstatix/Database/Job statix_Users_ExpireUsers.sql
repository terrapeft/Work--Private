...
IF NOT EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'statix_Users_ExpireUsers')
BEGIN
	/****** Object:  Job [tdApi_Users_ExpireUsers]    Script Date: 10/22/2014 14:03:02 ******/
	BEGIN TRANSACTION
	...
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'statix_Users_ExpireUsers', 
			@enabled=1, 
			...
			@owner_login_name=N'ARCADIA\Vitaly.Chupaev', @job_id = @jobId OUTPUT
	IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Zero counters', 
			...
			@command=N'exec [Statix].[dbo].[spJobExpireUsers]', 
			@database_name=N'Statix', 
			@flags=0
	IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Every day at 00:00', 
			...
			@schedule_uid=N'354a9948-27bc-463b-baf9-1167bee6e177'
	IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
