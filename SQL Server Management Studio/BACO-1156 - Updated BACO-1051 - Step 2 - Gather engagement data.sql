...
select distinct U.uUsername COLLATE Latin1_General_CI_AS as uUsername, s.sid
INTO ##subscribers 
from 
...
--PRINT 'STAGE 2 STARTING - ' + CONVERT(VARCHAR(20), GETDATE(),113)           
      ...
      -- Stage 2: Get mail logs
            SELECT      m.CampaignID, m.mIndex, m.TimeSent, m.EmailAddress, m.Undeliverable, m.Unsubscribed, m.ReturnedMailReason, s.sID 
...
--STAGE 3 ENDING - 30 Mar 2015 17:55:02
  ...
  -- PRINT 'STAGE 5 STARTING - ' + CONVERT(VARCHAR(20), GETDATE(),113)           
      ...
      CREATE TABLE dbo.ExportData(NewsletterId int, Pubwiz_PubId int, Pubwiz_PubName varchar(255), Pubwiz_PubShortName varchar(50),
                                  NewsletterName varchar(50), SiteID int, SiteName varchar(200), CampaignID int, NcuSubscrId int, 
								  BusinessId int, Newsletter varchar(200),
  ...
  --select count(*) from emailcampaign.dbo.exportdata