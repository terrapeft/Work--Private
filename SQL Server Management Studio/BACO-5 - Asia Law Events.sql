...
BEGIN TRY
    ...
    DECLARE @scID INT;
    DECLARE @sdID INT;
	DECLARE @PublicationName NVARCHAR(MAX) = 'Asia Law Events';
	DECLARE @PublicationURL NVARCHAR(MAX) = 'events.asialaw.com';
	DECLARE @ClaimName NVARCHAR(MAX) = 'registrant';
	DECLARE @SubscriptionName NVARCHAR(MAX) = 'Asia Law Events Registration';
	DECLARE @iiPublicationId INT = 57;
	...
	*/
    ...
    ,1
    ,1
    ,'MQE' -- same as for Asia Law (9305)
    ,11 -- same as for Asia Law (9305)
    ,228 -- same as for Asia Law (9305)
    ,GETDATE()
    ,'C')
