...
set @steps = ceiling(@usersCount/@selectByN)
set @stepValue = ceiling(100/@steps)
...
begin /* MAIN */
	...
	begin
		begin try
			begin transaction
			delete top (4500) t1 from [dbo].[MPUserFavourites] T1 join [dbo].[EventNcuUsers] T2 on T1.[EventNcuUserId] = T2.[euID] join #UsersToCheck U on T2.[euUserId] = U.[UserID]
			select @deleteCount = @@rowcount
			-- print @deleteCount
			commit transaction with (delayed_durability = on)
		end try
	...
	begin
		begin try
			begin transaction
			delete top (4500) t1 from [dbo].[EventNcuUsers] T1 join [dbo].[Orders] T2 on T1.[euOrderID] = T2.[oId] join #UsersToCheck U on T2.[oUID] = U.[UserID]
			select @deleteCount = @@rowcount
			-- print @deleteCount
			commit transaction with (delayed_durability = on)
		end try
	...
	-- print 'dbo, subscriptions_change_Stage'
	set @deleteCount = 4500
	while @deleteCount = 4500
	begin
		begin try
			begin transaction
			delete top (4500) t1 from [dbo].[subscriptions_change_Stage] T1 join #UsersToCheck U on T1.[suid] = U.[UserID]
			select @deleteCount = @@rowcount
			-- print @deleteCount
			commit transaction with (delayed_durability = on)
		end try
